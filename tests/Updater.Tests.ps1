...
try {
    ...
    if (-not $available.updateAvailable -or $available.currentVersion -ne '1.4.0' -or $available.latestVersion -ne '1.4.2') {
        throw 'Update check did not identify version 1.4.2 as newer than 1.4.0.'
    }
...
'@)
    ...
    try {
        ...
        $mutexRejected = $false
        try {
            & $updater -InstallRoot $blockedRoot -SourceArchive $blockedArchive -ExpectedArchiveSha256 ([string]$available.archiveSha256) -ExpectedVersion '1.4.2' -DeleteSourceArchive -MutexTimeoutSec 1 -NoRestart
    ...
    $current = & $updater -InstallRoot $installRoot -SourceArchive $archivePath -CheckOnly -CurrentVersion '1.4.2' | ConvertFrom-Json
    ...
    $badHashFailed = $false
    try {
        & $updater -InstallRoot $installRoot -SourceArchive $archivePath -ExpectedArchiveSha256 ('0' * 64) -ExpectedVersion '1.4.2' -NoRestart
    ...
    $newerAppText = (Get-Content -Raw -LiteralPath $installedApp) -replace '\$script:AppVersion\s*=\s*''1\.4\.2''', '$script:AppVersion = ''1.5.0'''
    ...
    try {
        & $updater -InstallRoot $installRoot -SourceArchive $archivePath -ExpectedArchiveSha256 ([string]$available.archiveSha256) -ExpectedVersion '1.4.2' -NoRestart
    ...
    $checkFailure = $false
    try { & $updater -InstallRoot $installRoot -SourceArchive $invalidArchive -CheckOnly -CurrentVersion '1.4.2' }
