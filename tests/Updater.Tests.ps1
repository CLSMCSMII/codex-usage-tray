...
try {
    ...
    $available = & (Join-Path $projectRoot 'src\Updater.ps1') -InstallRoot $installRoot -SourceArchive $archivePath -CheckOnly -CurrentVersion '1.2.0' | ConvertFrom-Json
    if (-not $available.updateAvailable -or $available.currentVersion -ne '1.2.0' -or $available.latestVersion -ne '1.2.1') { throw 'Update check did not identify a newer version.' }
    $current = & (Join-Path $projectRoot 'src\Updater.ps1') -InstallRoot $installRoot -SourceArchive $archivePath -CheckOnly -CurrentVersion '1.2.1' | ConvertFrom-Json
