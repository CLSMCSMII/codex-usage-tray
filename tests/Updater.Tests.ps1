...
try {
    ...
    $available = & (Join-Path $projectRoot 'src\Updater.ps1') -InstallRoot $installRoot -SourceArchive $archivePath -CheckOnly -CurrentVersion '1.2.2' | ConvertFrom-Json
    if (-not $available.updateAvailable -or $available.currentVersion -ne '1.2.2' -or $available.latestVersion -ne '1.2.3') { throw 'Update check did not identify a newer version.' }
    $current = & (Join-Path $projectRoot 'src\Updater.ps1') -InstallRoot $installRoot -SourceArchive $archivePath -CheckOnly -CurrentVersion '1.2.3' | ConvertFrom-Json
    ...
    foreach ($relative in @('src\CodexUsageTray.ps1', 'src\Updater.ps1', 'Launcher.vbs', 'Install.ps1', 'Uninstall.ps1', 'README.md', 'LICENSE')) {
